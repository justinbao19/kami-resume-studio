...
当前目录定义 13 类 × 2 主题 = 26 个可路由变体，详见 `references/resume-template-catalog.json`：

| 类型 | 适合岗位 |
| --- | --- |
| 纸序 Editorial | 产品、战略、咨询、综合 |
...
| 冰川履历 Aqua Ledger | 战略、咨询、精细运营与服务岗位 |
| 灰廊雅集 Atelier Serif | 高管、品牌与长篇叙事 |
| 库比蒂诺 Cupertino | 产品、软件、AI 与人本科技 |
| 经纬网格 Swiss Grid | 设计、创意与现代主义岗位 |
...
2. 13 类模板全部进入网页编辑器，并支持逐类切换浅色/深色主题。
3. 中文与英文标签、中文与英文示例数据。
4. 模板推荐配色、四种自定义强调色、两种主题和两种排版密度。
5. 统一照片上传入口，并按模板能力启用或禁用；照片只保存在当前浏览器，ATS companion 始终不带照片。
6. LinkedIn、X、GitHub 可选链接；技术、创意和早期职业模板使用图标，其余模板使用平台名与用户名，HTML/PDF 保留超链接。
7. 项目经历可新增和删除，并支持项目名称、角色或技术栈、时间、链接、简介和量化成果。
8. 浏览器本地自动保存，不上传个人数据。
9. 内容完成度、量化证据、项目证据和联系方式检查。
10. 桌面三栏工作台与移动端内容、预览、样式切换。
...
| 冰川履历 | 战略、咨询、精细运营、服务岗位 | 支持 | 图标 |
| 灰廊雅集 | 高管、品牌、叙事表达 | 支持 | 文字 |
| 库比蒂诺 | 产品、软件、AI、人本科技 | 支持 | 文字 |
| 经纬网格 | 设计、创意、现代主义岗位 | 支持 | 文字 |

照片和社媒都是可选信息。用户未主动提供或启用时，模板不会输出占位链接。项目经历则是全部模板都支持的一级模块，可用于放大产品案例、开源贡献、研究成果或跨职能项目。
