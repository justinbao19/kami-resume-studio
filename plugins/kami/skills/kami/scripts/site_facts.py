...
# Legacy Kami locale pages are retained as historical public surfaces. The
# Resume landing pair and editor are intentionally independent from them.
SITE_BASE_PAGE = "index.html"
SITE_LOCALE_PAGES = ()
RESUME_LANDING_PAGES = ("index.html", "index-en.html")
RESUME_EDITOR_PAGE = "editor.html"
...
FULL_PUBLIC_FACT_FILES = (
    ...
    "llms.txt",
    "index.md",
    *RESUME_LANDING_PAGES,
)
REDIRECT_SITE_FILE = RESUME_EDITOR_PAGE
...
def site_fact_issues(files: Mapping[str, str] | None = None) -> list[str]:
    ...
    editor = texts.get(REDIRECT_SITE_FILE)
    if editor is None:
        if files is not None:
            issues.append(f"{REDIRECT_SITE_FILE}: missing editor page")
    else:
        if 'class="workspace"' not in editor or 'id="resumePreview"' not in editor:
            issues.append(f"{REDIRECT_SITE_FILE}: missing Resume Studio editor markers")
