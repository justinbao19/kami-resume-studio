...
- Slate Sidebar is the exception to global paper and accent choices. Its light palette is fixed by role: sidebar `#EBEDF0`, paper `#F6F3F2`, shallow surfaces `#EFF1F0`, and ink or icons `#232935`. Keep all four roles synchronized across browser preview, print, and Agent output.
- Personal links are optional and must carry real `https://` hyperlinks. The editor supports LinkedIn, GitHub, X, Behance, Dribbble, Medium, GitLab, and labelled custom links. Technical, Creative, Early Career, and Slate Sidebar use compact official platform marks with accessible labels; Slate Sidebar keeps them in a horizontal row. The remaining families use platform text plus handle.
- Project experience is a first-class resume section. Keep each project row borderless and `break-inside: avoid`; include project name, role or stack, period, optional link, one-line context, and evidence-rich bullets. Prefer one to three selected projects over an exhaustive project inventory.
