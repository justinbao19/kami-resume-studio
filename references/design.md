...
- Personal photos are opt-in and market-sensitive. The web editor exposes a photo slot only for Editorial, Technical, Creative, and Early Career. ATS Classic, Executive, Sales Impact, Operations Practical, and Academic stay photo-free. The ATS companion remains photo-free unless the target market explicitly requires a photo.
- Resume photos use a fixed `1:1` slot, `object-fit: cover`, and `object-position: center 35%`. Store local editor uploads in the browser, never as a silent external upload.
