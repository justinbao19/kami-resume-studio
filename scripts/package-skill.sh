...
# Include local, uncommitted skill inputs during development as well as tracked
# files. Generated/forbidden surfaces are filtered below.
...
awk '
  ...
  /^scripts\/(build|check-update|checks|content|ensure-fonts|highlight|lint|mcp_server|mermaid_normalize|optional_deps|render|resume_workflow|shared|site_facts|tokens|verify|visual)\.(py|sh)$/ { print; next }
